...
public:
	...
	CheckOutItem getItem(int pos);
	CheckOutItem* subList(int len);
};
...
void Receipt::addItem(CheckOutItem item)
{
	list[position] = item;
	position++;
}

CheckOutItem Receipt::getItem(int pos)
{
	return list[pos];
}

CheckOutItem* Receipt::subList(int len)
{
	CheckOutItem* result = new CheckOutItem[len];
	for(int i=0; i < len; i++)
	{
		result[i] = list[i];
	}
...
int main()
{
	...
	Receipt now;
	CheckOutItem milkItem = CheckOutItem("Milk", 500);
	now.addItem(milkItem);
	now.add("Pasta", 300); // both ways work the same
	
	CheckOutItem* firstItem = now.subList(1);
	cout << firstItem[0] << endl;
	
	delete [] firstItem; // only thing you need to delete as deconstructors will do the rest
